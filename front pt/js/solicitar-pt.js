...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 4. Regra de negócio SESMT: OS Obrigatória vs Emergencial
    configurarRegraEmergencial();

    // 5. Carrega os colaboradores para busca por Matrícula
    await carregarBaseColaboradores();

    // 6. Configura a busca ao digitar a Matrícula
    const inputMatricula = document.getElementById('inputMatricula');
    if (inputMatricula) {
        inputMatricula.addEventListener('input', buscarPorMatricula);
    }

    // 7. Configura o botão de Adicionar Executante
    ...
    // 8. Configura o Checkbox "Eu (Solicitante) também vou atuar como Executante"
    ...
    // 9. Configura o envio do Formulário
    ...
    // 10. Caixa alta automática para o campo Turno / Grupo
    const inputTurnoGrupo = document.getElementById('turnoGrupo');
    if (inputTurnoGrupo) {
        inputTurnoGrupo.addEventListener('input', (e) => {
            e.target.value = e.target.value.toUpperCase();
        });
    }

    // 11. Caixa alta automática para o campo TAG / Equipamento
...
// =========================================================================
// REGRA DE NEGÓCIO SESMT: OS OBRIGATÓRIA vs ATENDIMENTO EMERGENCIAL
// =========================================================================
function configurarRegraEmergencial() {
    const chkEmergencial = document.getElementById('emergencial');
    const inputOS = document.getElementById('ordemPj');
    const asteriscoOS = document.getElementById('asteriscoOS') || document.getElementById('labelObrigatorioOS');

    if (!chkEmergencial || !inputOS) return;

    chkEmergencial.addEventListener('change', () => {
        if (chkEmergencial.checked) {
            inputOS.required = false;
            inputOS.placeholder = "Opcional (Atendimento Emergencial)";
            if (asteriscoOS) asteriscoOS.style.display = 'none';
        } else {
            inputOS.required = true;
            inputOS.placeholder = "Ex: OS-98421";
            if (asteriscoOS) asteriscoOS.style.display = 'inline';
        }
    });
}
...
async function enviarSolicitacaoPT(event) {
    ...
    // Monta o payload completo da PermissaoTrabalho.java incluindo a matriz detalhada
    const payload = {
        ...
        emergencial: document.getElementById('emergencial')?.checked || false,
        ...
        // Flips de Riscos Críticos (Booleans tradicionais)
        ...
        // MATRIZ DETALHADA DE RISCOS (RiskControlItem)
        ruido: {
        ...
        agentesQuimicos: {
            ativo: document.getElementById('quimicosAtivo')?.checked || false,
            opcoesSelecionadas: [],
            textoComplementar: document.querySelector('input[name="agentesQuimicos.textoComplementar"]')?.value.trim() || ""
        },
        diphoterine: {
            ativo: document.getElementById('diphoterineAtivo')?.checked || false,
            opcoesSelecionadas: [],
