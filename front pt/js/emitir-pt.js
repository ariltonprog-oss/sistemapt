    ...
    try {
        ...
        if (tbodyExecutantes) {
            ...
            if (pt.solicitanteExecutante && pt.solicitante) {
                ...
                if (!jaTemSolicitante) {
                    listaExecutantesRenderizar.push(pt.solicitante);
                }
            }
            ...
            if (listaExecutantesRenderizar.length > 0) {
                tbodyExecutantes.innerHTML = listaExecutantesRenderizar.map(e => `
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                        <td style="padding: 8px;"><strong>${e.matricula || 'N/A'}</strong></td>
                        <td style="padding: 8px;">${e.nome || 'Não informado'}</td>
                        <td style="padding: 8px;">${e.funcao || e.cargo || 'Operacional'}</td>
                    </tr>
                `).join('');
            } else {
                tbodyExecutantes.innerHTML = `
                    <tr>
                        <td colspan="3" style="padding: 12px; text-align: center; color: var(--texto-secundario, #a0aec0);">
                            Nenhum executante especificado para esta PT.
                        </td>
                    </tr>`;
            }
        }
